...
ihre Dateien hat der Merge nicht berührt. Phase 91b (Issue #91, Plan 91b-10) hat in `employees.ts`,
`salon-assignments.ts`, `avatars.ts`, `availability.ts`, `shift-patterns.ts`, `settings.ts`,
`leave-settings.ts` und `leave.ts` neue Scope-Prüfungen VOR den bestehenden, hier zitierten
Handler-Zeilen eingefügt — jede dieser Dateien hat dadurch verschobene Zeilennummern, absichtlich
NICHT neu gezählt (der Vollständigkeitstest prüft Anzahl und Permission je Datei, nicht die
Zeilennummer selbst, s. u.); nur die **Prose** der PERSON-Ressourcen und die MANDANT-Zeilen wurden
für den jetzt durchgesetzten Scope aktualisiert. Phase 76b (Issue #76) hat den neuen Abschnitt
...
die Mechanik der Scope-Durchsetzung (#91, seit Phase 91b umgesetzt: `contexts/platform/
scope-filter.ts`, ADR 0001-abweichungen Eintrag O) — nur DASS eine `ZUGEWIESEN`-Permission auf einer
`PERSON`-Ressource seitdem auf den Scope der Zuweisung eingeschränkt ist (Mandant, Salons oder
Personenliste), steht unten je Ressource.
...
Mandant (seit Phase 91b durchgesetzt, #91). Bei einer `PERSON`-Ressource wirkt `ZUGEWIESEN` seitdem
nur auf die Mitarbeiter im Scope der Zuweisung: eine Mandant-Zuweisung auf alle, eine Salon-Zuweisung
auf die Mitarbeiter mit Stammsalon zum Stichtag (bei Mitarbeiter-Stammdaten zusätzlich mit aktivem
Einsatzsalon), eine Personenliste auf genau die gelisteten Mitarbeiter.
...
- Eine Permission auf einer Ressource mit Bezug Mandant wirkt nur bei einer Zuweisung mit Scope
  Mandant (seit Phase 91b durchgesetzt, #91).
- Eine Permission auf einer Ressource mit Bezug Person wirkt seit Phase 91b (#91) nur auf die
  Mitarbeiter im Scope der Zuweisung — die einzelnen Zeilen unten nennen es nicht mehr gesondert,
  wo sie schon „im Scope“ sagten: das war von Anfang an so gemeint und ist es jetzt auch technisch.
...
| Permission                  | erlaubt                                                                                                                                                                                  | erlaubt ausdrücklich nicht                                                                                                                                                    |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `holiday:manage:ZUGEWIESEN` | Feiertage anlegen und löschen (`POST /holidays`, `DELETE /holidays/:id`) sowie Schulferien einsehen und neu laden (`GET /admin/school-holidays`, `POST /admin/school-holidays/refresh`). | Betriebsurlaub (`company-shutdown:manage`); Berufsschultage (`vocational-school:manage`). Wirkt nur bei einer Zuweisung mit Scope Mandant (seit Phase 91b durchgesetzt, #91). |

### `salon` — Salons

| Permission                | erlaubt                                                                                | erlaubt ausdrücklich nicht                                                                                                                                                                                                                 |
| ------------------------- | -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `salon:read:ZUGEWIESEN`   | Die Salons des Mandanten und ihre Angaben lesen, über die Endpunkte, die #64 einführt. | Salons anlegen, ändern oder schließen (`salon:manage`); die Daten der Mitarbeiter eines Salons — dafür gelten die Permissions der Personen-Ressourcen. Wirkt nur bei einer Zuweisung mit Scope Mandant (seit Phase 91b durchgesetzt, #91). |
| `salon:manage:ZUGEWIESEN` | Salons anlegen, ändern und schließen, über die Endpunkte, die #64 einführt.            | Den Scope einer Rollenzuweisung auf Salons setzen (`role-assignment:manage`); Mitarbeiterdaten ändern. Wirkt nur bei einer Zuweisung mit Scope Mandant (seit Phase 91b durchgesetzt, #91).                                                 |
...
Salonmanager, Personalabteilung, Ausbilder (§ Systemrollen-Templates unten). Sie gelten
mandantenübergreifend (`tenantId` leer), haben feste Ids (`SYSTEM_ROLE_IDS` in
`contexts/platform/system-roles.ts`) und werden im Code an der Id erkannt, nie am Namen. Sie lassen
sich nicht ändern und nicht löschen (409), nur kopieren. Der drei-Rollen-Rest dieses Absatzes gilt
unverändert für Admin, Manager und Mitarbeiter; ihre Permissions sind aus der Spalte
...
| Permission                          | erlaubt                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | erlaubt ausdrücklich nicht                                                                                                                                                                                                                                  |
| ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `role-assignment:manage:ZUGEWIESEN` | Rollen an Personen zuweisen, ändern und entziehen, samt Scope der Zuweisung (Mandant, Salons oder Personenliste): Zuweisungen auflisten (`GET /role-assignments`), eine lesen (`GET /role-assignments/:id`), eine Rolle mit Scope zuweisen (`POST /role-assignments`), Rolle oder Scope ändern (`PATCH /role-assignments/:id`) und entziehen (`DELETE /role-assignments/:id` — löschen, auditiert). Auch die Rolle im Mitarbeiterformular (`PATCH /employees/:id` mit `role`) braucht diese Permission (`employees.ts:738`): Sie ersetzt seit Phase 75b die Systemrollen-Zuweisung des Nutzers, statt `User.role` zu setzen — seit Phase 76b (P-01) nur noch eine Zuweisung auf Admin, Manager oder Mitarbeiter; eine Template-Zuweisung (Inhaber, Salonmanager, Personalabteilung, Ausbilder, § Systemrollen-Templates) übersteht das Formular unverändert. Eine angeforderte Rolle, die der aktuellen `User.role`-Spalte des Ziels gleicht, löst seit Plan 76b-08 (D-16) gar keine Rollenänderung aus. Nur mit Scope Mandant: seit Phase 91b (D-05) strukturell erfüllt — `role-assignment` hat Bezug Mandant, eine Salons- oder Personenliste-Zuweisung gewährt `role-assignment:manage:ZUGEWIESEN` seitdem gar nicht mehr (der Zugriffs-Gate liest für Mandant-Ressourcen nur die enge `zugewiesen`-Menge, nie die erweiterte `zugewiesenAnyScope`-Menge), es gibt hier also nichts zusätzlich einzuschränken. Seit Issue #354 (Pre-Merge-Sicherheitsreview zu #75) außerdem Voraussetzung für: eine Rolle außer Mitarbeiter beim Anlegen (`employees.ts:457`) oder Importieren (`imports.ts:157`) vergeben; einen API-Schlüssel mit Scope `admin` anlegen (`api-keys.ts:63`); eine selbst innegehabte Rolle ändern (`roles.ts:271`) — jeweils weil die Aktion sonst eine Rechteausweitung wäre, die nur `employee:create`/`employee:import`/`api-key:manage`/`role:manage` allein nicht abdecken. | Rollen selbst definieren (`role:manage`); Stammdaten ändern (`employee:update`); den Nutzer einer bestehenden Zuweisung austauschen (dafür entziehen und neu zuweisen). Wirkt nur bei einer Zuweisung mit Scope Mandant (seit Phase 91b durchgesetzt, #91). |
...
| Permission                       | erlaubt                                                                                                                                  | erlaubt ausdrücklich nicht                                                                                                                                                                           |
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `leave-config:read:ZUGEWIESEN`   | Die Konfiguration der Abwesenheitsarten des Mandanten lesen (`GET /settings/leave-types`).                                               | Abwesenheitsarten oder Sonderurlaubsregeln ändern (`leave-config:manage`); Anträge (`leave-request:read`). Wirkt nur bei einer Zuweisung mit Scope Mandant (seit Phase 91b durchgesetzt, #91).       |
| `leave-config:manage:ZUGEWIESEN` | Abwesenheitsarten ändern (`PUT /settings/leave-types/:id`) und Sonderurlaubsregeln anlegen, ändern und löschen (`/special-leave/rules`). | Urlaubsansprüche einzelner Mitarbeiter (`leave-entitlement:update`); Betriebsurlaub (`company-shutdown:manage`). Wirkt nur bei einer Zuweisung mit Scope Mandant (seit Phase 91b durchgesetzt, #91). |

### `company-shutdown` — Betriebsurlaub

| Permission                           | erlaubt                                                                                                                         | erlaubt ausdrücklich nicht                                                                                                                                                      |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `company-shutdown:manage:ZUGEWIESEN` | Betriebsurlaub anlegen, ändern und löschen und einzelne Mitarbeiter davon ausnehmen (`/company-shutdowns`, `…/:id/exceptions`). | Einzelne Urlaubsanträge genehmigen (`leave-request:approve`); Feiertage (`holiday:manage`). Wirkt nur bei einer Zuweisung mit Scope Mandant (seit Phase 91b durchgesetzt, #91). |
...
| `overtime:read:ZUGEWIESEN`                | Salden, Monatssalden und Monatsstände der Mitarbeiter im Scope lesen; seit Phase 76b (#76, D-15) auch den Überstunden-Trend des Dashboards (`GET /dashboard/overtime-trend`), aggregiert über die Mitarbeiter im Scope; seit Phase 76b (#76, D-06) zusammen mit `team-overview:read` auch die Saldenübersicht (`GET /dashboard/overtime-overview`) — beide Reichweiten werden geschnitten, eine engere überstimmt die weitere nie. | Tageszeiten und einzelne Zeiteinträge (`time-entry:read`); Vertragsdaten (`contract:read`); den Saldo verändern (`overtime:settle`, `overtime:set-opening-balance`); nichts außerhalb des Scopes der Zuweisung. |
...
| Permission                       | erlaubt                                                                                                           | erlaubt ausdrücklich nicht                                                                                                                                                                |
| -------------------------------- | ----------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `shift-config:manage:ZUGEWIESEN` | Schichtvorlagen und Besetzungsregeln anlegen, ändern und löschen (`/shifts/templates`, `/shifts/coverage-rules`). | Schichten planen (`shift:plan`); Schicht-Wochenmuster einzelner Mitarbeiter (`shift-pattern:update`). Wirkt nur bei einer Zuweisung mit Scope Mandant (seit Phase 91b durchgesetzt, #91). |
...
| Permission                      | erlaubt                                                                                                                                                                                                                                                                                                                                                                                                   | erlaubt ausdrücklich nicht                                                                                                                                                                                     |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `team-overview:read:ZUGEWIESEN` | Team-Übersichten für die Mitarbeiter im Scope sehen: Wochenansicht, Anwesenheit heute, offene Punkte des Teams und Team-Ereignisse der Aktivitätsansicht (`GET /dashboard/team-week`, `…/today-attendance`, `…/open-items`, `GET /activity`). Die Saldenübersicht (`…/overtime-overview`) erfordert seit Phase 76b (#76, D-06) zusätzlich `overtime:read` — team-overview:read allein zeigt keine Salden. | Einzelne Zeiteinträge, Anträge oder Salden über die Übersicht hinaus (`time-entry:read`, `leave-request:read`, `overtime:read`); den Audit-Feed (`audit-log:read`); nichts außerhalb des Scopes der Zuweisung. |

## Systemrollen-Templates

Seit Phase 76b (Issue #76) gibt es vier weitere, GLOBALE Systemrollen — Templates, keine
zuweisbaren Kundenrollen im engeren Sinn: feste Ids (`SYSTEM_ROLE_IDS` in
`contexts/platform/system-roles.ts`), nicht änderbar und nicht löschbar (409, wie jede
Systemrolle), nur kopierbar (`POST /roles/:id/copy`). Der Scope gehört immer der Zuweisung (#74),
nicht der Rolle — die Spalte „vorgesehener Scope" unten nennt nur die Absicht, es gibt keine
API-Bindung eines Templates an einen Scope-Typ. Wer als Salonmanager, Personalabteilung oder
Ausbilder selbst arbeitet, bekommt zusätzlich die Systemrolle Mitarbeiter mit Reichweite EIGENE
(Scope Mandant) — das hält diese drei Templates frei von EIGENE-Rechten und von der
Vier-Augen-Kombination (#78). Inhaber ist der volle Katalog; die Permissions-Zelle wird deshalb
gegen die LEBENDE Katalog-Aufzählung geprüft (`system-roles.test.ts`), nicht gegen eine feste
Liste — eine neue Katalog-Permission macht diesen Test rot, bis jemand für den Inhaber entscheidet.

| Template          | Id                                     | vorgesehener Scope | Permissions                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| ----------------- | -------------------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Inhaber           | `00000000-0000-4000-8000-00000000a004` | Mandant            | jede Permission des Katalogs                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| Salonmanager      | `00000000-0000-4000-8000-00000000a005` | Salons             | `employee:read:ZUGEWIESEN`, `time-entry:read:ZUGEWIESEN`, `time-entry:create:ZUGEWIESEN`, `time-entry:update:ZUGEWIESEN`, `time-entry:delete:ZUGEWIESEN`, `time-entry:revalidate:ZUGEWIESEN`, `retro-request:read:ZUGEWIESEN`, `retro-request:create:ZUGEWIESEN`, `retro-request:approve:ZUGEWIESEN`, `leave-request:read:ZUGEWIESEN`, `leave-request:create:ZUGEWIESEN`, `leave-request:approve:ZUGEWIESEN`, `leave-request:attest:ZUGEWIESEN`, `leave-request:cancel:ZUGEWIESEN`, `section9:read:ZUGEWIESEN`, `section9:upload:ZUGEWIESEN`, `section9:decide:ZUGEWIESEN`, `leave-entitlement:read:ZUGEWIESEN`, `vocational-school:read:ZUGEWIESEN`, `shift:read:ZUGEWIESEN`, `shift:plan:ZUGEWIESEN`, `shift-pattern:read:ZUGEWIESEN`, `availability:read:ZUGEWIESEN`, `team-overview:read:ZUGEWIESEN` |
| Personalabteilung | `00000000-0000-4000-8000-00000000a006` | Mandant            | `employee:read:ZUGEWIESEN`, `employee:create:ZUGEWIESEN`, `employee:update:ZUGEWIESEN`, `employee:update-avatar:ZUGEWIESEN`, `contract:read:ZUGEWIESEN`, `contract:update:ZUGEWIESEN`, `leave-request:read:ZUGEWIESEN`, `section9:read:ZUGEWIESEN`, `leave-entitlement:read:ZUGEWIESEN`, `leave-entitlement:update:ZUGEWIESEN`, `vocational-school:read:ZUGEWIESEN`, `overtime:read:ZUGEWIESEN`, `month-close:read:ZUGEWIESEN`                                                                                                                                                                                                                                                                                                                                                                           |
| Ausbilder         | `00000000-0000-4000-8000-00000000a007` | Personen           | `employee:read:ZUGEWIESEN`, `time-entry:read:ZUGEWIESEN`, `leave-request:read:ZUGEWIESEN`, `vocational-school:read:ZUGEWIESEN`, `shift:read:ZUGEWIESEN`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |

**Salonmanager — nicht enthalten, mit Grund (D-05):**

- `leave-request:correct` — korrigiert einen bereits genehmigten Antrag und berechnet den Saldo
  neu; das bleibt beim Inhaber (wer es dem Salonmanager geben will, kopiert das Template).
- `overtime:*`, `month-close:*`, `report:*` — Saldo, Monatsabschluss und der Monatsbericht (er
  enthält Saldo) bleiben Inhaber/Personalabteilung vorbehalten (#85).
- `contract:*` — Vertragsdaten sind kein Salon-Thema.
- Jede MANDANT-Relations-Permission (etwa `salon:*`) — sie wirkt ohnehin nur mit Scope Mandant.
- `shift-pattern:update`, `availability:update` — vertragsnah; Planung läuft über `shift:plan`.
- `vocational-school:manage`, `leave-entitlement:update`.

**Personalabteilung — nicht enthalten, mit Grund (D-07):**

- Jede `time-entry:*`/`retro-request:*`-Permission, `presence-source:*`, `terminal:*` — Tageszeiten
  sind ein Bewegungsprofil (Datenminimierung, Art. 5 DSGVO).
- Jede `shift:*`/`shift-pattern:*`/`availability:*`-Permission, `team-overview:read` — Anwesenheit
  ist ebenfalls ein Bewegungsprofil.
- `report:*` — `report:read` bündelt den Monatsbericht MIT Tageszeilen; damit fällt auch die
  Urlaubsübersicht vorerst weg (Aufteilung: #86).
- Jede Genehmigungs-Permission (`approve`/`decide`/`attest`/`correct`/`cancel`) — „jede
  Genehmigung" bleibt bei Inhaber/Salonmanager.
- `employee:anonymize` — die DSGVO-Löschung bleibt beim Inhaber.
- `employee:import` — der CSV-Import setzt Rollen über die Spalte `rolle`.
- `employee:manage-access` — Login/Rollen.
- `overtime:settle`, `overtime:set-opening-balance`, `month-close:close`/`unlock`/`close-year`.
- Jede MANDANT-Permission.

`leave-entitlement:update` ist enthalten, weil der Urlaubsanspruch Vertragsbestandteil ist;
`section9:read` ist enthalten als Teil von „Abwesenheiten lesen", obwohl es sich um
Gesundheitsdaten nach Art. 9 DSGVO handelt — die Personalabteilung ist die übliche Halterin
ärztlicher Bescheinigungen.

**Ausbilder — nicht enthalten, mit Grund (D-08):** alles außer den fünf gelisteten Lese-Rechten,
...
(Zeiten genehmigen). Die Selbstgenehmigung bleibt trotzdem zur Laufzeit gesperrt (#78, keine
Permission). Admin und Manager halten heute ebenfalls beide Hälften — das prüft #78, nicht diese
Phase.

**Kompatibilitätsrolle (D-10).** Eine Inhaber- oder Personalabteilung-Zuweisung mit Scope Mandant
ergibt `User.role = MANAGER` (nur die Admin-Id ergibt `ADMIN`); eine Salonmanager- oder
Ausbilder-Zuweisung mit Scope Salons oder Personen trägt nichts zur Kompatibilitätsrolle bei
(`compat-role.ts`, unverändert).

**Das Mitarbeiterformular (P-01).** Die Rollen-Brücke im Mitarbeiterformular
(`PATCH /employees/:id` mit `role`) ersetzt seit Phase 76b nur noch eine gespeicherte Zuweisung
auf Admin, Manager oder Mitarbeiter (`isLegacySystemRoleId()`) — eine Template-Zuweisung
(Inhaber, Salonmanager, Personalabteilung, Ausbilder) übersteht einen Formular-Speichervorgang
unverändert und zählt weiterhin als Sperre gegen eine Herabstufung auf Mitarbeiter (409). Eine
angeforderte Rolle, die der aktuellen `User.role`-Spalte des Ziels gleicht, ist außerdem seit
Plan 76b-08 KEINE Rollenänderung (D-16): Ein unverändertes Speichern der Stammdaten löst die
Brücke gar nicht erst aus, sodass ein Personalabteilung- oder Kundenrollen-Halter keine
...
| `composition/dashboard.ts:865`                             | `GET /overtime-overview`                        | A, M    | `team-overview:read`           | ZUGEWIESEN                                   |
| `composition/dashboard.ts:873`                             | `GET /overtime-overview`                        | A, M    | `overtime:read`                | ZUGEWIESEN                                   |
| `composition/dashboard.ts:1481`                            | `GET /overtime-trend`                           | A, M    | `overtime:read`                | ZUGEWIESEN                                   |
...
Beginnt die Reichweite mit `EIGENE`, fragt die Stelle nur die eigene Reichweite ab. Die
ZUGEWIESEN-Reichweite derselben Permission wird dann entweder an einer anderen Stelle derselben
Route geprüft (`contexts/absence/api/leave.ts:370`, Issue #359) oder genügt dort ausdrücklich
nicht: Das persönliche Dashboard (`composition/dashboard.ts:96` und `:97`, ergänzt im Code-Review
von Phase 76b) liefert ohne `overtime:read:EIGENE` bzw. `leave-entitlement:read:EIGENE` `null`
statt Saldo bzw. Urlaubstage, auch wenn der Aufrufer die ZUGEWIESEN-Reichweite hält —
`hasPermission` prüft den vollständigen Schlüssel. Hintergrund: „Saldo-Gates“ im Abschnitt
„Systemrollen-Templates“.
...
| `composition/dashboard.ts:96`                            | `GET /`                               | A, M, E                                       | `overtime:read`          | EIGENE; sonst `null` statt Saldo — ZUGEWIESEN genügt nicht       |
| `composition/dashboard.ts:97`                            | `GET /`                               | A, M, E                                       | `leave-entitlement:read` | EIGENE; sonst `null` statt Urlaubstage — ZUGEWIESEN genügt nicht |
...
| `contexts/platform/facade/role-assignments.ts:197` | `resolveAccessReach` (Phase 91b, Issue #91, D-03/D-08): der Altrollen-Rückfall für einen Nutzer ohne gespeicherte Rollenzuweisung liest die Spalte `User.role` und übergibt sie an `systemRoleIdForLegacyRole`, wie in `request-permissions.ts:143`; das Ergebnis bestimmt nur, ob die aufgelöste `AccessReach` `wholeTenant` oder `scoped` ist — keine eigene Rollenentscheidung.                                                                                                                                                                                                                                                                                               |
| `contexts/platform/facade/role-assignments.ts:470` | `userIdsHoldingPermission` (D-16): der Altrollen-Rückfall (D-08) liest die Spalte `User.role` eines Nutzers ohne gespeicherte Zuweisung und übergibt sie an `systemRoleIdForLegacyRole`, wie in `request-permissions.ts:143`; keine eigene Rollenentscheidung. (Zeile verschoben von :352 auf :470 durch Phase 91b Plan 01 (Issue #91): `resolveAccessReach` wurde oberhalb dieser Funktion in dieselbe Datei eingefügt.)                                                                                                                                                                                                                                                        |
