...
`apps/api/scripts/__tests__/context-area-map.test.ts` — every one of the 109 non-test files under
`routes/`, `utils/` and `plugins/` is keyed there, and an unmapped file throws `UnmappedFileError`.
That map assigns a CONTEXT AREA (the bucket); this document's table below additionally states the
concrete DESTINATION PATH each file moves to, which is one level more specific and is what
`apps/api/scripts/context-cut-move.ts` (the wave-1 codemod, temporary — deleted by plan 99B-08)
is populated from. The two files' six `[from, to]` tables agree LITERALLY with the table below —
proof obligation P7 in section 6 is exactly that literal comparison.

129 files move in total: 105 non-test source files and 54 co-located tests (D-07: a test stays next
...
### Machine-generated flat list (agrees literally with `context-cut-move.ts`)

Generated by `pnpm --filter @clokr/api exec tsx scripts/context-cut-move.ts --print-all-pairs | sort`
— this is the exact text plan 99B-01's own proof compares against with `diff` (section 6, P7). If
this block and the codemod's six `const` tables ever disagree, one of them is wrong and the diff is
what catches it.
...
plan: `pnpm --filter @clokr/api exec tsx scripts/context-cut-move.ts --print-all-pairs | sort` and a
`grep -oE` extraction of this document's own machine-generated block above, `diff`ed, print nothing.
