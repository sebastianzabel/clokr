...
describe("Leave correction (PATCH /requests/:id/correct)", () => {
  ...
  // "unresolvable leaveType name → 400 instead of a silent booking skip (IN-94-01)" removed
  // (Issue #206): its fixture built a codeless LeaveType row to force `oldTypeCode` undefined —
  // `LeaveType.code` is `NOT NULL` in the database now (migration
  // 20260923090815_leave_type_code_not_null), so that row can no longer exist and the 400 guard
  // it exercised (`leave.ts`'s `correct` handler, formerly `if (!oldTypeCode || !newType)`) was
  // removed in the same commit as unreachable. The assertion moved from application code into
  // the database constraint, proven directly against Postgres in Task 1 of this issue's plan
  // (SQLSTATE 23502 on a null-code insert) rather than through this test.
});
