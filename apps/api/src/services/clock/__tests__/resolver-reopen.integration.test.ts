...
// Issue #136 batch E — ONE clock read for the whole file. buildEvent() and the Test 3 / Test 5
// seeds used to derive "today" independently; a UTC midnight between those two reads gave the
// seeded row a different day-key than the events, the resolver correctly created a SECOND
// per-day entry, and the "exactly one active row" invariant failed for a reason unrelated to the
// code under test. Noon UTC so every offset used here (<= 8h) stays on the same UTC day.
// resolveClockEvent reads no clock of its own — every date it uses arrives on the ClockEvent —
// so a single anchor makes this file fully deterministic with respect to wall-clock time.
const ANCHOR = (() => {
  const d = new Date();
  d.setUTCHours(12, 0, 0, 0);
  return d;
})();
const ANCHOR_DATE_STR = ANCHOR.toISOString().slice(0, 10);
...
describe("services/clock/resolver — D-01 reopen path (resolver-reopen.integration)", () => {
  ...
  function buildEvent(opts: { source: string; intent: ClockIntent; hoursAgo: number }): ClockEvent {
    ...
    // Batch E (#136): now shares the single module-level ANCHOR with the Test 3 / Test 5 seeds
    // below, instead of taking its own independent `new Date()` read.
    const timestamp = new Date(ANCHOR.getTime() - opts.hoursAgo * 3600 * 1000);
  ...
  it("locked closed entry → CONFLICT MONTH_LOCKED, entry stays closed", async () => {
    ...
    // Batch E (#136): derives from the shared ANCHOR instead of its own `new Date()` read, so
    // this seed and buildEvent() can never disagree about the calendar day.
    const lockedEntry = await app.prisma.timeEntry.create({
      data: {
        employeeId: data.employee.id,
        date: new Date(`${ANCHOR_DATE_STR}T00:00:00.000Z`),
        startTime: new Date(ANCHOR.getTime() - 4 * 3600 * 1000),
        endTime: new Date(ANCHOR.getTime() - 3 * 3600 * 1000),
  ...
  it("WR-01 regression: zero-duration artifact in DB is NOT picked as consolidation predecessor", async () => {
    ...
    // Batch E (#136): derives from the shared module-level ANCHOR instead of its own `new
    // Date()` read, so this seed can never disagree with buildEvent()/Tests 1-4 about the day.
    ...
    const t1 = new Date(ANCHOR.getTime() - 8 * 3600_000); // 8h ago
    ...
    const t2 = new Date(ANCHOR.getTime() - 6 * 3600_000); // 6h ago — real startTime
    const t3 = new Date(ANCHOR.getTime() - 0.1 * 3600_000); // 6 min ago — just closed
    const fakeOpenEntry = {
  ...
  // ── Test 6: Issue #136 permanent regression — a real midnight crossing mid-sequence ──────
  it("issue #136: a UTC midnight between the seed and the reopen event does not create a second entry", async () => {
    // Pin the clock just before UTC midnight of ANCHOR's own date (never an absolute literal —
    // derived from ANCHOR so this test can never expire), run IN/OUT, advance the clock PAST
    // midnight, then run the reopen IN. resolveClockEvent reads no clock of its own (every date
    // arrives on the ClockEvent), so this reproduces exactly the two-read race the seeds above
    // used to have: if any code path under test derived "today" a second time from the live
    // clock, this is where it would disagree with the events and create a second TimeEntry row.
    const pinned = new Date(`${ANCHOR_DATE_STR}T23:59:30.000Z`);
    try {
      ...
      // Cross UTC midnight mid-sequence.
      vi.setSystemTime(new Date(pinned.getTime() + 90 * 1000));
