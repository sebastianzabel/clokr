...
describe("measure-context-coverage — perFileCoverageRows", () => {
  it("sorts by area (CONTEXT_AREAS order) then ascending line %", () => {
    const summary: CoverageSummary = {
      total: fileEntry([10, 10], [4, 4]),
      [`${API_ROOT}/src/routes/time-entries.ts`]: fileEntry([100, 80], [10, 8]), // zeiterfassung, 80%
      [`${API_ROOT}/src/utils/arbzg.ts`]: fileEntry([100, 20], [10, 2]), // zeiterfassung, 20%
    ...
    expect(rows.map((r) => r.relPath)).toEqual([
      "src/utils/arbzg.ts", // zeiterfassung, thinnest first
      "src/routes/time-entries.ts", // zeiterfassung, then thicker
