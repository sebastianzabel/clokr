...
| backfill-leave-type-code.ts             | Removed 2026-09 (Issue #206) | Backfill + repeatable post-rollout sweep for `LeaveType.code IS NULL` rows (Phase 97). Issue #206's migration (`packages/db/prisma/migrations/20260923090815_leave_type_code_not_null`) made that state permanently impossible in the database, so the script's only selection (`code IS NULL`) can never again match a row, on any environment. Its 10-case test suite (`scripts/__tests__/backfill-leave-type-code.test.ts`) could not be repaired to match — every case first constructed a codeless row, which the database now refuses. See `docs/migrations.md` § "Phase 97 — LeaveType.code (Rollout-Reihenfolge)" steps 3/5 for the rollout history this replaces. |
