...
- **Unterbau** (Shared Kernel, `contexts/platform/`) — the `platform` models of `MODEL_OWNER` in `apps/api/scripts/measure-foreign-context-access.ts` (Tenant, TenantConfig, Employee, User, WorkSchedule, …) and permissions. Salon (#64) and Salonzuordnung (#67) land here; Beschäftigung (#66) is backlog and has no entity.
